...
// Listen for messages from external web pages (the React app)
chrome.runtime.onMessageExternal.addListener((message, sender, sendResponse) => {
  ...
  switch (message.type) {
    ...
    case 'SYNC_CONTEXTS':
      // Sync contexts from web app to extension
      syncContextsFromApp(message.contexts).then(sendResponse);
      return true;
...
// Sync contexts from web app
async function syncContextsFromApp(newContexts) {
  if (!Array.isArray(newContexts)) {
    return { success: false, error: 'Invalid contexts data' };
  }
  
  const result = await chrome.storage.local.get(['contexts']);
  let existingContexts = result.contexts || [];
  
  // Merge contexts - update existing ones, add new ones
  for (const newContext of newContexts) {
    const existingIndex = existingContexts.findIndex(c => c.id === newContext.id);
    if (existingIndex !== -1) {
      // Update existing context if newer
      if (newContext.updatedAt > existingContexts[existingIndex].updatedAt) {
        existingContexts[existingIndex] = newContext;
      }
    } else {
      // Add new context
  ...
  await chrome.storage.local.set({ contexts: existingContexts });
  
  // Notify all AI chat tabs about the update
  notifyAllTabs();
  
  return { success: true, contexts: existingContexts };
}

// Notify all tabs about context changes
async function notifyAllTabs() {
  const tabs = await chrome.tabs.query({});
  for (const tab of tabs) {
    try {
      await chrome.tabs.sendMessage(tab.id, { type: 'CONTEXTS_UPDATED' });
    } catch (e) {
